  ...
  <section class="space-y-3">
    ...
    <div class="space-y-3 md:hidden">
      ...
      <article v-for="row in rows" v-else :key="row.id" class="rounded-xl border border-gray-800 bg-gray-900 p-4" @click="$emit('details', row)">
        <div class="flex items-start justify-between gap-3"><div class="min-w-0"><StatusBadge :status="row.leave_type_name" :variant="getLeaveTypePresentation(row.leave_type_name).variant">{{ getLeaveTypePresentation(row.leave_type_name).label }}</StatusBadge><p class="mt-2 text-xs text-gray-400">{{ formatRange(row) }}</p></div><StatusBadge :status="row.status" /></div>
    ...
    <AppTable :loading="loading" class="hidden md:block">
      ...
      <tbody class="divide-y divide-gray-800 bg-gray-900">
        <tr v-for="row in rows" :key="row.id" class="cursor-pointer hover:bg-gray-950" @click="$emit('details', row)"><td class="table-cell text-primary-200">{{ formatRange(row) }}</td><td class="table-cell whitespace-nowrap">{{ formatDate(row.created_at, true) }}</td><td class="table-cell"><StatusBadge :status="row.leave_type_name" :variant="getLeaveTypePresentation(row.leave_type_name).variant">{{ getLeaveTypePresentation(row.leave_type_name).label }}</StatusBadge></td><td class="table-cell text-xs">{{ payLabel(row) }}</td><td class="table-cell"><StatusBadge :status="row.status" /></td><td class="table-cell text-right" @click.stop><div class="flex flex-wrap justify-end gap-2"><AppButton variant="secondary" size="sm" @click="$emit('details', row)">Details<span v-if="row.unread_comment_count" class="h-2 w-2 rounded-full bg-red-500" /></AppButton><AppButton v-if="row.status === 'pending'" variant="secondary" size="sm" @click="$emit('edit', row)">Edit</AppButton><AppButton v-if="row.status === 'pending'" variant="danger" size="sm" @click="$emit('cancel', row)">Cancel</AppButton><StatusBadge v-if="pendingByLeaveId.has(Number(row.id))" status="pending">{{ pendingByLeaveId.get(Number(row.id)).request_type === 'move' ? 'Move pending' : 'Cancellation pending' }}</StatusBadge><AppButton v-else-if="canRequestChange(row)" variant="secondary" size="sm" @click="openChangeRequest(row)">Request change</AppButton></div></td></tr>
